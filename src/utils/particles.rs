...
pub fn init_particles(canvas_ref: NodeRef<leptos::html::Canvas>) {
    canvas_ref.on_load(move |canvas| {
        ...
        let perticles: Vec<Particle> = (0..count).map(|i| {
            let color_ind = i % COLORS.len();
            Particle {
        ...
        render_loop(canvas, ctx, perticles, mouse, is_mobile);
    });
}
        ...
        for i in 0..particles.len() {
          ...
          for j in (i+1)..particles.len() {
             ...
             if dist_sq < connection_dist * connection_dist {
                ...
                ctx.move_to(px, py); ctx.line_to(op.x, op.y);
                ctx.stroke();
                particles[i].connections += 1 ; particles[j].connections += 1;
             }
          }
